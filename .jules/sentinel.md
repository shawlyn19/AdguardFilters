...
**Prevention:** Regularly run `npm audit` and use the `overrides` field in `package.json` to pin secure versions of transitive dependencies. Maintain strict line ending compliance for `package.json` and `package-lock.json` in this repository to avoid noisy diffs.
